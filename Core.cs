...
[assembly: MelonInfo(typeof(DoubleJump.Core), "DoubleJump", "1.0.0", "Coolbriggs", null)]
...
namespace DoubleJump
{
    ...
    public class Core : MelonMod
    {
        ...
        // Config file paths
        private static readonly string CONFIG_DIRECTORY = Path.Combine("UserData", "DoubleJump");
        private static readonly string CONFIG_FILE = Path.Combine(CONFIG_DIRECTORY, "config.json");
        ...
        public override void OnInitializeMelon()
        {
            ...
            LoggerInstance.Msg("Double Jump mod initialized!");
        ...
        private void SyncConfigToClients()
        {
            ...
            try
            {
                ...
                string configData = $"{CONFIG_MESSAGE_PREFIX}{config.JumpForce}|{config.EnableDoubleJump}|{config.MaxJumpHeight}|{config.AllowTripleJump}";
        ...
        private static class SteamCallbacks
        {
            ...
            private static void ProcessReceivedConfigMessage(string message)
            {
                ...
                try
                {
                    ...
                    if (parts.Length >= 4)
                    {
                        config.JumpForce = float.Parse(parts[0]);
                        config.EnableDoubleJump = bool.Parse(parts[1]);
                        config.MaxJumpHeight = float.Parse(parts[2]);
                        config.AllowTripleJump = bool.Parse(parts[3]);
        ...
        public override void OnPreferencesSaved()
        {
            ...
            float newJumpForce = MelonPreferences.GetEntryValue<float>(CATEGORY_GENERAL, SETTING_JUMP_FORCE);
            ...
            bool newEnableDoubleJump = MelonPreferences.GetEntryValue<bool>(CATEGORY_GENERAL, SETTING_ENABLE_DOUBLE_JUMP);
            if (newEnableDoubleJump != config.EnableDoubleJump)
            {
                config.EnableDoubleJump = newEnableDoubleJump;
                configChanged = true;
            }
            
            float newMaxJumpHeight = MelonPreferences.GetEntryValue<float>(CATEGORY_GENERAL, SETTING_MAX_JUMP_HEIGHT);
            ...
            bool newAllowTripleJump = MelonPreferences.GetEntryValue<bool>(CATEGORY_GENERAL, SETTING_ALLOW_TRIPLE_JUMP);
            if (newAllowTripleJump != config.AllowTripleJump)
            {
